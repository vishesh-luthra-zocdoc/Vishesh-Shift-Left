# provider-fe-monorepo — Integration (integration) Billing Tests
...
**Zero billing tests cross a real boundary.** Stated plainly, because an empty integration tier in a frontend is itself the
finding:
...
field rename, a status-code change, or a route-template typo is caught only at the browser level (Playwright) or in production.
See `api-coverage.md` for the endpoint-by-endpoint view; the API contract tier is also empty (0 files).

What *does* exist at the integration level is **in-process composition**: two files that render a multi-component tree through a real
React context/provider with two or more real collaborators and no browser. Under the CONVENTIONS.md rule
("multi-component tree with a real provider/context" → Integration) they qualify, and they are catalogued below — but they are
a weaker kind of integration test than the definition's "real infra" examples, and I flag them as such rather than letting the
count imply boundary coverage.

## Integration files (2 files, 70 declared blocks)
...
**Why this counts as an integration test, not a component test.** It renders the real page shell inside a real `MemoryRouter` (`:21`, `:298`) and the container
...
itself is covered by the second integration-test file.
...
**Why this counts as an integration test, not a component test.** It renders the real `PayNowModal` → real `ReviewStep` → real core `FailedPaymentsByMonth`
...
swallows rejection), with inline-error behaviour deferred to `CardEntryStep-tests.tsx` (Component). So **no test renders
the real Stripe card entry inside the real Pay Now tree** — the highest-risk composition in the billing product
(entering a new card to clear a failed balance) is split across an integration tree with a stubbed card step and a component card
step with stubbed Stripe.

## Borderline component/integration — flagged, counted as component tests

These four render 2+ real collaborators but a single component under test, so I kept them at the component level. Listing them so the
Integration count is not read as "only two files compose real code".

| File | Kind assigned | Real collaborators | Why not an integration test |
|---|---|---|---|
| `PaymentRecovery/steps/__tests__/ReviewStep-tests.tsx` (12 blocks) | Component | **Zero `jest.mock`.** Real core `FailedPaymentsByMonth` → real `FailedPaymentMethodCard` → real `recoveryCopy` | One component under test with props; no provider/context and no data layer at all. Closest file in the scope to the integration level without being it |
| `__tests__/billing-analytics-events-tests.tsx` (11 blocks) | Component | Real `BillingSettingsContext.Provider`, real `initializeMetricsDataOnWindow`, five real components (`FAQsSection`, `MarketplaceCard`, `PaymentMethodsListV2`, `BillingContactInfo`, `Bill`) | Renders the five independently rather than as one tree; the concern is analytics wiring, not composition |
| `shared/core/src/components/AddPaymentMethodModal/__tests__/BillingAddressFields-tests.tsx` (5 blocks) | Component | Real `useBillingAddressForm` (react-hook-form + `billingAddressSchema`) | Single component, one mock (dropdown) |
| `shared/core/src/billing/__tests__/useBillingAddressPrefill-tests.ts` (4 blocks) | Hook | Real `useBillingAddressForm` + real schema, only `getBillingAddress` mocked | Hook under test, not a tree |
| `shared/core/src/components/FailedPaymentsByMonth/__tests__/FailedPaymentsByMonth-tests.tsx` (13 blocks) | Component | **Zero mocks**; real `FailedPaymentMethodCard` + real copy helpers | Single component under test with props |

## Candidate Gaps

| # | What's missing | Level | Path(s) | Why it matters | Effort | Priority |
|---|---|---|---|---|---|---|
| INT-1 | **No MSW (or any) request-boundary layer exists.** Nothing between "module mocked in Jest" and "Playwright against production" | Integration | Whole scope; `apps/settings/.../apiCalls.ts`, `apps/settings/src/config/routes.ts:562–683`, `shared/core/src/billing/billingApiClient.ts` | Every request shape, route template, and response mapping on the billing money path is validated only against mocks the tests themselves author. Backend field renames and status-code changes are invisible until a browser test runs. Standing up MSW once unlocks real coverage for all 12 route templates | M (setup) + S per suite | **P0** |
| INT-2 | No test renders the real Stripe card entry inside the real Pay Now tree — `PayNowModal-tests.tsx:108` stubs `CardEntryStep`, and `CardEntryStep-tests.tsx` stubs `@stripe/react-stripe-js` and `@stripe/stripe-js` | Integration | `PaymentRecovery/__tests__/PayNowModal-tests.tsx`, `steps/__tests__/CardEntryStep-tests.tsx`, `shared/core/src/testing/installStripeJsFake.ts` | The declined-card recovery flow is the product's highest-consequence path; the composition of card entry + charge is asserted nowhere below the browser level. `installStripeJsFake.ts` (130 lines) already exists and is used only by `apps/settings/e2e/fixtures.ts` and a Storybook story | M | **P0** |
| INT-3 | No test composes an app consumer with the real core `AddPaymentMethodElementModal` — both consumers stub it (`AddPaymentMethodModalV2-tests.tsx:30`, `ActivationAddPaymentMethodModal-tests.tsx:31`) | Integration | those two files + `shared/core/src/components/AddPaymentMethodModal/AddPaymentMethodElementModal.tsx` | The Stripe Payment Element extraction created a package seam with tests on both sides and nothing across it; a prop-contract break ships | M | **P1** |
| INT-4 | `BillingSettingsContainer-tests.tsx` mocks the core `RecoveryStatusBanner` (:205) even though the real one is a zero-mock component with its own 9-block suite | Integration | `__tests__/BillingSettingsContainer-tests.tsx:205` | Un-mocking it would make the banner-placement and suppression describes assert real rendered copy instead of a stub, at near-zero cost | XS | P2 |
| INT-5 | Pure derivations asserted through a full-page integration render: `rollover-blocked ids derived from the payment-method list (BILL-1017)` and parts of `recovery banner suppression` | Unit | `__tests__/BillingSettingsContainer-tests.tsx:885,935` | Slowest possible place to test a pure function; extract the derivation and cover it at the unit level, leave the wiring assertion at the integration level | S | P2 |
| INT-6 | The only two integration-test files are both in `apps/settings`; `shared/core`, `provider-home-webapp` and `spo-webapp` have **no integration tests at all** | Integration | `shared/core/src/**`, `apps/provider-home-webapp/src/**`, `apps/spo-webapp/src/**` | `shared/core` is the package every consumer depends on, and it is verified only as isolated units and components | M | P2 |
...
|---|---|---|---|---|
| unit | 34 | 224 | 49 | 73 |
| component render | 45 | 515 | 43 | 112 |
| Hook hook render | 9 | 47 | 4 | 12 |
| **integration (this file)** | **2** | **70** | **6** | **15** |
...
Integration by area: `apps/settings` 2 files / 70 blocks. `shared/core` 0. `apps/provider-home-webapp` 0. `apps/spo-webapp` 0.
Both integration-test files are among the four largest test files in the scope (1141 and 796 lines; only `FpbInvoiceView-tests.tsx`
