...
## Kinds of test (MANDATORY — use these exact labels)
...
|---|---|---|---|
| Unit | `unit` | One function/module in isolation. No React render, no HTTP, no DB, no browser. Pure logic, schemas, formatters, reducers, C# classes with all deps mocked. | Does it render a component or touch a boundary? If no → Unit. |
| Component | `component` | Renders ONE component in isolation (RTL/`render()`), props and hooks mocked. Fast, in-process, no real network. | Uses `render()`/`screen` but mocks its data layer → Component. |
| Integration | `integration` | Multiple real units together across a real boundary: real DB, real in-memory HTTP pipeline, LocalStack, MSW-backed fetching, multi-component tree with a real provider/context. No browser. | Real infra or 2+ real collaborators, still no browser → Integration. |
| API contract | `api` | Exercises an HTTP surface and asserts the request/response contract (status, schema, headers). In-memory host or deployed endpoint. | Asserting on an HTTP contract → API contract. |
| Browser | `e2e` | Real browser driving a running app in a real environment. Playwright, Selenium/WebDriver. | Browser involved → Browser. |

Notes:
- A Jest file under `__tests__/` is NOT automatically a unit test. Read it. Most `*-tests.tsx` in
  this monorepo are **component**, not unit. Classify per `describe`/`it` block if a
  single file mixes levels, and say so.
- zocdoc_web `BillingTests`/`StripeTests` drive a browser → those are **browser**, even though
